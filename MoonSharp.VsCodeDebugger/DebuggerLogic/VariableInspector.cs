...
namespace MoonSharp.VsCodeDebugger.DebuggerLogic
{
	internal static class VariableInspector
	{
		internal static void InspectVariable(DynValue v, List<Variable> variables)
		{
			...
			variables.Add(new Variable("(val #id)", v.ReferenceID.ToString(CultureInfo.InvariantCulture)));
			...
			switch (v.Type)
			{
				...
				case DataType.Function:
					variables.Add(new Variable("(address)", v.Function.EntryPointByteCodeLocation.ToString("X8", CultureInfo.InvariantCulture)));
					variables.Add(new Variable("(upvalues)", v.Function.GetUpvaluesCount().ToString(CultureInfo.InvariantCulture)));
				...
				case DataType.Table:
					...
					variables.Add(new Variable("(table #id)", v.Table.ReferenceID.ToString(CultureInfo.InvariantCulture)));

					if (v.Table.MetaTable != null)
						variables.Add(new Variable("(metatable #id)", v.Table.MetaTable.ReferenceID.ToString(CultureInfo.InvariantCulture)));

					variables.Add(new Variable("(length)", v.Table.Length.ToString(CultureInfo.InvariantCulture)));
				...
				case DataType.Thread:
					...
					variables.Add(new Variable("(auto-yield counter)", v.Coroutine.AutoYieldCounter.ToString(CultureInfo.InvariantCulture)));
					break;
